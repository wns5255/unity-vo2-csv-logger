...
public class InternalRotation : BaseRehabilitationEvaluator
{
    // 운동 단계를 정의합니다.
    ...
    public InternalRotation(ObjectData objdata, int totalset,TMP_Text achivedText,CircleFill circleFill)
    ...
    public override void RealtimeEvaluate(int currentset)
    {
        ...
        if (!ismove)
        {
            ...
            {
                if (Vector2.Distance(wristPosQueue.Peek(), currentWristPos) > 0.05f) // 선택된 팔의 데이터 사용
                {
                    ismove = true;
                    idleTimer_2=0f;
                    idleTimer=0f;
        ...
        if(idleTimer_2>3.0f){
            ...
            currentEvaluationSide==EvaluationSide.Right&&currentWristPos.x>(POSITION_TOLERANCE+0.1f)&&Math.Abs(currentWristPos.y)-0.5f<POSITION_TOLERANCE){
                MessageManager.Instance.DisplayMessage("팔을 너무 벌리지 말아주세요.");
                // FeedbackImageManager.Instance.ShowBadImage();
                int badCount = PlayerPrefs.GetInt("BadCount", 0) + 1;
                PlayerPrefs.SetInt("BadCount", badCount);
                idleTimer_2 = 0f;
        ...
        switch (currentState)
        {
            case ExerciseState.Idle:
                if (isTargetPoseMaintained && !hasreached)
                {
                    ...
                    idleTimer = 0f;
                    holdTimer = 0f; // 타이머를 초기화하고 유지 상태로 전환합니다.
