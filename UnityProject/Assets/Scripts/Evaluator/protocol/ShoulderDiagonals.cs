...
public class ShoulderDiagonals : BaseRehabilitationEvaluator
{
    ...
    public ShoulderDiagonals(ObjectData objdata, int totalset,TMP_Text achivedText,CircleFill circleFill)
    ...
    public override void RealtimeEvaluate(int currentset)
    {
        ...
        float angleRad60 = 52.5f * Mathf.Deg2Rad;
        float cos60 = Mathf.Cos(angleRad60);
        float sin60 = Mathf.Sin(angleRad60);
        ...
        {
            elbowTargetPosition = new Vector2(-0.5f * cos45, 0.5f * sin45);
            wristTargetPosition = new Vector2(-0.95f * cos45, 0.95f * sin45);

            // 기본 준비 자세
            elbowReturnPosition = new Vector2(0.1f, -0.5f);
            wristReturnPosition = new Vector2(0.8f * cos60, -0.8f * sin60);
        ...
        else // EvaluationSide.Right
        {
            ...
            // 기본 준비 자세
            elbowReturnPosition = new Vector2(-0.1f, -0.5f);
            wristReturnPosition = new Vector2(-1.0f * cos60, -1.0f * sin60);
            ...
            case ExerciseState.Idle:
                ...
                else if (!hasreached)
                {
                    ...
                    if (idleTimer > 4.0f)
                    {
                        ...
                        // 현재 위치를 운동 경로(직선)에 투영하여 가장 가까운 지점을 찾습니다.
